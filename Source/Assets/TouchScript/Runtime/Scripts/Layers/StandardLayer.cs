...
namespace TouchScript.Layers
{
    ...
    public class StandardLayer : TouchLayer
    {
        ...
        private static Dictionary<int, ProjectionParams> projectionParamsCache = new();
        private static List<BaseRaycaster> raycasters;

        private static List<RaycastHitUI> raycastHitUIList = new(20);
        private static List<RaycastHit> raycastHitList = new(20);
        private static List<HitData> hitList = new(20);
...
#pragma warning restore CS0414
        ...
        public override ProjectionParams GetProjectionParams(Pointer pointer)
        {
            ...
            if (!projectionParamsCache.TryGetValue(canvas.GetInstanceID(), out var pp))
            {
                // TODO: memory leak
        ...
        private HitResult performWorldSearch(IPointer pointer, out HitData hit)
        {
            ...
            if (!_camera.enabled || !_camera.gameObject.activeInHierarchy) return HitResult.Miss;
        ...
        private void performUISearchForCanvas(IPointer pointer, Canvas canvas, GraphicRaycaster raycaster, Camera eventCamera = null, float maxDistance = float.MaxValue, Ray ray = default)
        {
            var position = pointer.Position;
