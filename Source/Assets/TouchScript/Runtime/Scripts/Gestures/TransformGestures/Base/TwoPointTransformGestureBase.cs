...
namespace TouchScript.Gestures.TransformGestures.Base
{
    ...
    public abstract class TwoPointTransformGestureBase : TransformGestureBase
    {
                ...
                if (newScreenDelta.sqrMagnitude > minScreenPointsPixelDistanceSquared)
                {
                    if (rotationEnabled)
                    {
                        ...
                        else
                        {
                            ...
                            TwoD.PointToLineDistance2(oldScreenPos1, oldScreenPos2, newScreenPos1, newScreenPos2, out var d1, out var d2);
        ...
        protected virtual float doRotation(Vector2 oldScreenPos1, Vector2 oldScreenPos2, Vector2 newScreenPos1, Vector2 newScreenPos2, ProjectionParams projectionParams) => 0;
        ...
        /// <param name="projectionParams"> Layer projection parameters. </param>
        /// <returns> Delta translation vector. </returns>
        protected virtual Vector3 doTwoPointTranslation(Vector2 oldScreenPos1, Vector2 oldScreenPos2, Vector2 newScreenPos1, Vector2 newScreenPos2, float dR, float dS, ProjectionParams projectionParams) => Vector3.zero;

        /// <summary>
        /// Gets the number of points.
        /// </summary>
        /// <returns> Number of points. </returns>
        protected virtual int getNumPoints() => NumPointers;
        ...
        protected virtual Vector2 getPointScreenPosition(int index) => activePointers[index].Position;

        /// <summary>
        /// Returns previous screen position of a point with index 0 or 1
        /// </summary>
        /// <param name="index"> The index. </param>
        protected virtual Vector2 getPointPreviousScreenPosition(int index) => activePointers[index].PreviousPosition;
        ...
        protected virtual void drawDebug(int touchPoints)
        {
            ...
            {
                case 1:
                    GLDebug.DrawSquareScreenSpace(debugID, getPointScreenPosition(0), 0f, debugPointerSize, color, float.PositiveInfinity);
                    GLDebug.RemoveFigure(debugID + 1);
                    GLDebug.RemoveFigure(debugID + 2);
                    break;
                default:
                    var newScreenPos1 = getPointScreenPosition(0);
                    var newScreenPos2 = getPointScreenPosition(1);
                    GLDebug.DrawSquareScreenSpace(debugID, newScreenPos1, 0f, debugPointerSize, color, float.PositiveInfinity);
                    GLDebug.DrawSquareScreenSpace(debugID + 1, newScreenPos2, 0f, debugPointerSize, color, float.PositiveInfinity);
                    GLDebug.DrawLineWithCrossScreenSpace(debugID + 2, newScreenPos1, newScreenPos2, .5f, debugPointerSize * .3f, color, float.PositiveInfinity);
                    break;
            }
        }
