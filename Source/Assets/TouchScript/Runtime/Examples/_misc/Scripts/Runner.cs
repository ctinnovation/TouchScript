    ...
    public class Runner : MonoBehaviour
    {
        ...
        private void Start()
        {
...
#if UNITY_EDITOR
            var guids = AssetDatabase.FindAssets("t:Scene", new[] {"Assets/TouchScript/Examples"});
            if (EditorBuildSettings.scenes.Length != guids.Length)
            {
                ...
                {
                    var importers = Array.ConvertAll(guids, guid => AssetImporter.GetAtPath(AssetDatabase.GUIDToAssetPath(guid)));
                    Array.Sort(importers, (a, b) =>
                    {
                    ...
                    EditorBuildSettings.scenes = Array.ConvertAll(importers, i => new EditorBuildSettingsScene(i.assetPath, true));
